...
from .reduction import train_autoencoder, umap_reduce
...
def make_results(
    texts: List[str], embeddings_nd: np.ndarray, embeddings_3d: np.ndarray
):
    rows = []
...
def sample_texts(
    texts: List[str], sample_factor: Optional[float] = None, random_seed: int = 42
...
def run_pipeline(
    ...
    embeddings = compute_embeddings(
        texts, model_name=hf_model, batch_size=embed_batch_size, device=device
    )
