...
def select_device() -> str:
    ...
    for i, device in enumerate(devices):
        ...
        device_info = (
            f"{info.get('name', 'Unknown')} ({info.get('memory_gb', 0):.1f}GB)"
            if "memory_gb" in info
            else info.get("name", "Unknown")
        )
...
def configure_method() -> tuple:
    ...
    if method == "umap":
        ...
        hidden_str = Prompt.ask(
            "Hidden layer sizes (comma-separated)", default="256,128"
        )
...
def interactive_setup() -> Dict[str, Any]:
    console.print(
        Panel.fit("🚀 [bold]Embedder Interactive Setup[/bold] 🚀", style="blue")
    )
    ...
    viz_params = {}
    if visualize:
        viz_params["color_by_length"] = Confirm.ask(
            "Color by text length?", default=False
        )
