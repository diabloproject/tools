...
def train_autoencoder(
    ...
    logger.info(
        f"Training autoencoder on {device}: {embeddings.shape} -> {latent_dim}D"
    )
    ...
    model = SimpleAE(
        input_dim=embeddings.shape[1], hidden_dims=hidden_dims, latent_dim=latent_dim
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.train()
        for (batch,) in tqdm(
            loader, desc=f"Epoch {epoch}/{epochs}", disable=not verbose
        ):
